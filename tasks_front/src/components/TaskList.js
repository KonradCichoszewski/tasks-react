...
import { tryAddingTask, tryDeletingTask } from "../redux/actions/tasksActions";
...
class TaskList extends React.Component {
  ...
  render() {
    ...
    if (!this.props.loggedIn) {
      optionalMessage = (
        <p className="new_task_message">
          Log in or sign up in order to add new tasks
        </p>
      );
    }
        ...
        <div className="task_list_tile">
          ...
          {this.props.tasks.reverse().map((task) => (
            <Task
              task={task}
              key={task.id}
              delete={this.props.tryDeletingTask}
...
export default connect(mapStateToProps, { tryAddingTask, tryDeletingTask })(
  TaskList
);
