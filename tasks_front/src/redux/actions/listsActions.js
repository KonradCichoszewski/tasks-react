...
import { getTasks, getTasksSuccess, getTasksFailure } from "./tasksActions";
...
export function tryRenamingList(token, list_title, list_id) {
  return async (dispatch) => {
    ...
    await axios
      .patch(
        "http://localhost:3000/lists/" + list_id,
        {
          title: list_title,
        },
        {
...
export function changeCurrentList(list, token) {
  return async (dispatch) => {
    await dispatch(setCurrentList(list));
    dispatch(getTasks);
    axios
